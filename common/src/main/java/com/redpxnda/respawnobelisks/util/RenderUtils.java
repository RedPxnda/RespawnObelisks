...
public class RenderUtils {
    ...
    public static TextureAtlasSprite angelSprite;
    ...
    public static void renderRunes(RenderType renderType, TextureAtlasSprite sprite, float[] colors, RespawnObeliskBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            float zPos = (-RespawnObelisksConfig.INSTANCE.clientOptions.zPos) / 16f;

            addQuad(false, poseStack, vc, colors[0], colors[1], colors[2], (float) (blockEntity.getClientCharge()/blockEntity.getClientMaxCharge()), width/32f, 2*height/32f, zPos, 0, sprite.getU(texStartX), sprite.getU(texEndX), sprite.getV(texStartY), sprite.getV(texEndY), packedLight);
    ...
    public static void renderTotemItem(BlockEntityRendererProvider.Context context, RespawnObeliskBlockEntity blockEntity, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        itemRenderer.renderStatic(
                RespawnObelisksConfig.INSTANCE.revival.revivalItem.getDefaultInstance(),
