...
public class ScheduledRespawnAnchorTask {
    ...
    public final void tick() {
        if (this.tick++ >= this.tickDelay) {
            ...
            if (this.tick % 10 == 0)
                Packets.sendToPlayer(new RespawnAnchorInteractionPacket(this.pPos, false, charge), player);
        }
    }

    public void run() {
        player.level.setBlock(pPos, Blocks.AIR.defaultBlockState(), 3);
        Packets.sendToPlayer(new RespawnAnchorInteractionPacket(this.pPos, true, charge), player);
